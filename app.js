function updateLocation(url){try{window.history.replaceState(null,"",url)}catch{/* Navigation remains functional when History is unavailable. */}}
...
function setView(v){
 updateLocation(location.pathname+location.search);view=v;current=null;tab="overview";document.querySelectorAll(".nav").forEach(b=>b.classList.toggle("active",b.dataset.view===v));
...
function home(){
 ...
 <div class=card><h3>Explore the full research progression</h3><p>${window.LIFE_CODE_RESEARCH.research_records.length} historical records connect the original discoveries to later tests, failures, model revisions, and frozen predictions.</p><button class=action data-route="research">Follow the research journey →</button></div><div class=card><h3>Recovered evidence is now explorable</h3><p>${S.vocabulary.length} exact shared sequences, ${S.occurrences.length} verified focused locations, and an auditable correction trail now connect the original calibration to this Codebook.</p><button class=action data-route="recovery">Explore the recovered evidence →</button></div><div class="card plainOnly"><div class=question>Why would anyone care?</div><p>If useful structure can be recovered without telling the system what genes are supposed to matter, the Codebook can become a neutral map of recurring genomic structure rather than a list built from existing annotations.</p></div>
...
function selectObject(id){id=resolveObjectId(id);updateLocation("#object="+encodeURIComponent(id));view="codebook";current=id;tab="overview";document.querySelectorAll(".nav").forEach(b=>b.classList.toggle("active",b.dataset.view==="codebook"));setExplorer(true);renderList();renderObject()}
...
function integrity(){
 ...
 <div class=card><h3>Known open reconciliation items</h3><ul><li>PC0001 literals, 19 focused locations, and all 54 shared 16-base vocabulary records are now reconciled. The Research journey now publishes 113 historical records; complete biological graph normalization is still pending.</li><li>The older Proof Case 0001 k=16 null summary and the later full vocabulary-ladder k=16 summary require source-level reconciliation.</li></ul></div>
 <div class=downloads><a href="data/LIFE_CODE_PUBLIC_CODEBOOK_v0.11.json" download>Download public JSON</a><a href="downloads/LIFE_CODE_CODEBOOK_CORE_v0.11.sqlite" download>Download SQLite Codebook</a><a href="SCIENTIFIC_RELEASE_POLICY.md">Release policy</a></div>`;
}
function render(){if(view==="home")home();else if(view==="findings")findings();else if(view==="recovery")evidenceRecovery();else if(view==="research")researchLedger();else if(view==="researchRecord")showHistoricalRecord(current);else if(view==="codebook")current?renderObject():objectLanding();else if(view==="experiments")experiments();else if(view==="models")models();else if(view==="vocabulary")vocabulary();else if(view==="history")evolutionaryHistory();else if(view==="resilience")resilience();else if(view==="program")program();else if(view==="claims")claims();else if(view==="glossary")glossary();else integrity();bindTerms()}
...
const requestedResearch=new URLSearchParams(location.hash.slice(1)).get("research");if(requestedResearch)showHistoricalRecord(requestedResearch);
